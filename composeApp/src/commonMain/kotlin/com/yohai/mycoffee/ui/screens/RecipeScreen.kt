            ...
            Column(Modifier.padding(24.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                ExposedDropdownMenuBox(expanded, { expanded = it }) {
                    ...
                    ExposedDropdownMenu(expanded, { expanded = false }) {
                        BrewMethod.entries.forEach { option ->
                            DropdownMenuItem({ Text(formatBrewMethod(option)) }, {
                                method = option
                                expanded = false
                            })
                        }
                    ...
                    Button(onClick = {
                        onSave(
                            BrewRecipe(
                                ...
                                yield.toDoubleOrNull()?.times(if (useGrams) 1.0 else 28.3495),
                                time.toInt(),
                                temperature.toDoubleOrNull()?.times(if (useGrams) 1.0 else 28.3495),
                                notes.ifBlank { null },
                            ),
                        )
